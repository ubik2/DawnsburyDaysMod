        ...
        public static void RegisterSpells()
        {
            ModManager.ReplaceExistingSpell(SpellId.FireRay, 1, ((spellcaster, spellLevel, inCombat, spellInformation) =>
            {
                ...
                return Spells.CreateModern(IllustrationName.FireRay, "Fire Ray", new[] { Trait.Uncommon, Trait.Attack, Trait.Cleric, Trait.Concentrate, Trait.Fire, Trait.Focus, Trait.Manipulate },
                    ...
                    "Make a spell attack roll against the target's AC. The ray deals " + S.HeightenedVariable(2 + heightenIncrements, 2) + "d6 fire damage on a hit (or double damage on a critical hit).\n" +
                ...
                {
                    await CommonSpellEffects.DealAttackRollDamage(action, caster, target, checkResult, 2 * spellLevel + "d6", DamageKind.Fire);
                            ...
                            TileQEffect tileEffect = new TileQEffect(tile)
                            {
                                ...
                                StateCheck = (self) =>
                                {
                                    self.Owner.PrimaryOccupant?.AddQEffect(new QEffect(ExpirationCondition.Ephemeral)
                                    {
                                        ...
                                        {
                                            await creature.DealDirectDamage(null, DiceFormula.FromText(spellLevel + "d6", "Fire Ray fire tile"), creature, CheckResult.Failure, DamageKind.Fire);
                                        }
                                    });
                                }
                            };
